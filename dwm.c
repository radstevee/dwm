...
void
managealtbar(Window w, XWindowAttributes *wa)
{
  Monitor *m;
  ...
  XSelectInput(dpy, w,
               EnterWindowMask | FocusChangeMask | PropertyChangeMask |
                   StructureNotifyMask);
  XMoveResizeWindow(dpy, w, wa->x, wa->y, wa->width, wa->height);
  XMapWindow(dpy, w);
  XChangeProperty(dpy, root, netatom[NetClientList], XA_WINDOW, 32,
...
spawn(const Arg *arg)
{
  if (fork() == 0) {
    ...
    fprintf(stderr, "dwm: execvp '%s' failed:", ((char **)arg->v)[0]);
  }
}
...
void
resource_load(XrmDatabase db, char *name, enum resource_type rtype,
                   void *dst)
{
  char *sdst = NULL;
  int *idst = NULL;
